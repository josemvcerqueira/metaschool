module dex::faucet {
  ...
  // @dev These are utility functions so users can mint coins to swap

  /*
  * @param storage We pass a mutable reference of the Shared Object. It is a shared object so anyone poss to the function. The Mut keyword means that it can be mutated.  
  * @param ctx It is automatically passed as the last argument to all Sui functions. It contains metadata about the current Chain/TX Status
  */
